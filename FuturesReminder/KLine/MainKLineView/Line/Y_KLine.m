...
- (void)kLinePosition{
    //如果阴影线部分是实体部分2到3倍长度，则触发本地通知
    float shadowLineHeight = fabsf(self.kLinePositionModel.HighPoint.y  - self.kLinePositionModel.LowPoint.y);
    float entityLineHeight = fabsf(self.kLinePositionModel.OpenPoint.y - self.kLinePositionModel.ClosePoint.y);
