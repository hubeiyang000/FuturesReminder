...
- (UIButton *)private_createButtonWithTitle:(NSString *)title tag:(NSInteger)tag
{
    ...
    btn.titleLabel.font = [UIFont systemFontOfSize:13];
    btn.tag = tag;
    [btn addTarget:self action:@selector(event_buttonSelect:) forControlEvents:UIControlEventTouchUpInside];
    [btn setTitle:title forState:UIControlStateNormal];
    return btn;
}

#pragma mark 底部按钮点击事件
- (void)event_buttonSelect:(UIButton *)button{
   
    NSInteger index = button.tag-Y_StockChartSegmentStartTag;
    self.selectedIndex = index;
    [[NSNotificationCenter defaultCenter] postNotificationName:LineTypeChanged object:[NSNumber numberWithInteger:index]];
