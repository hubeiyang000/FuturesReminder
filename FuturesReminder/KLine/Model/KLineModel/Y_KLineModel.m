...
- (void) initWithDic:(NSDictionary *)dic;
{
    ...
    if (self)
    {
        ...
        NSDate *date= [_dateFormatter dateFromString:dic[@"d"]];
        _Date = [NSString stringWithFormat:@"%f",[date timeIntervalSince1970]];
        _Open = @([dic[@"o"] floatValue]);
        _High = @([dic[@"h"] floatValue]);
        _Low = @([dic[@"l"] floatValue]);
        _Close = @([dic[@"c"] floatValue]);

        _Volume = [dic[@"v"] floatValue];
